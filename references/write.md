...
### 7.5.2 复用 research 段已抓的竞品文案（v3 —— 去重复抓取）
...
**兜底**：
- 若 `input/product-info.txt` 不存在或字符数 < 200（research 段抓取彻底失败的极少数情况）→ 跳过对比循环，记 `compare: "skipped_source_empty"`，不阻塞 finalize
- 不会要求用户手动粘贴（以前 copy-compare 单独跑时需要手动粘是另一个场景，copy-workflow 编排内全自动）
