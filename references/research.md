...
OUTPUTS:   input/product-info.txt
           ...
           飞书多维表「调研报告」字段（fldFy3UkGV）已回填 docx URL（v4 新增）
...
app_token:  FLklbT5KPaa5e1swuwAc6ifGnXg  （strapi 文案任务表）  
table_id:   tblDHUN1vEbPqaoT  
view_id:    vewK3tr5O0  （strapi文案待生成）

若用户给的是 Wiki URL（含 `/wiki/<token>`），用：
...
- **品牌**: `品牌` (field_id: **`fldk2C4d3X`**, type: **text**，值直接是品牌名如 `"Sylovona Glow™"`) ★ **正式品牌来源**

> v6 简化（2026-04-29）：新表「品牌」从 link 改为 text，**取消跳品牌总表二次解析**。直接读主表行的 `品牌` 字段即可。`品牌参考` (`fldckCpJpq`) 是镜像字段，偶有延迟，不用。
...
const idxBrand   = fields.indexOf('品牌');   // text 字段，值直接是品牌名
const hits = [];
for (let i=0;i<rows.length;i++) {
  ...
  if (Array.isArray(c) ? c.includes('<country>') : c === '<country>') {
    hits.push({
      record_id: recIds[i],
      comp:  rows[i][idxComp],
      brand: rows[i][idxBrand]   // 品牌名文本，如 "Sylovona Glow™"
    });
  }
}
...
每个 hit 包含 `{ record_id, comp, brand }`，brand 已是文本，无需二次跳表。
...
- **【品牌】**：<brand，如 "Sylovona Glow™"，来自飞书主表"品牌"字段 `fldk2C4d3X`（text，直接是品牌名），不是镜像字段"品牌参考">
...
|---|---|---|---|
| 调研报告 | **`fldFy3UkGV`** | text（URL 自动包装 markdown 链接） | 调研报告 docx URL |
