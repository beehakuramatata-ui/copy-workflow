# references/poll-fill.md — 定时扫描+批量补跑文案段（v5.7）

> 主 agent 按本文件执行：扫描飞书产品总表 → 找出"调研报告已填 / 文案为空"的产品行 → **串行**触发 write 段（v5.6 后内置 finalize 全套）→ 自动回填飞书"文案"字段。
>
> **触发方式**：
> 1. 自动 — 由 `mcp__scheduled-tasks` 注册的 **2 小时**周期任务唤起（cron `17 */2 * * *`，避开整点拥堵）
> 2. 手动 — 用户跑 `/copy-workflow poll-fill` 立即扫一次
>
> **为什么是 2 小时不是 30 分钟**：单产品 ~12 min（write 一站式），3 个产品就 ~36 min 撞下次扫描。2 小时间隔够 8-10 个产品串行不撞车。
>
> **v5.7 与 poll-fill-research 的对偶关系**（完整自动化状态机，详见 `references/poll-fill-research.md`）：
>
> | 行状态 | 缺什么 | 谁负责 | 跑什么 |
> |---|---|---|---|
> | 1 | 基础信息空（国家/竞品/产品/品牌） | 人 | 用户手填飞书 |
> | 2 | 基础信息齐 + **调研报告空** | poll-fill-research（4h 周期，**默认未启用**） | research 段 → 飞书回填调研报告字段 |
> | 3 | 调研报告齐 + **文案空** | **本文件 poll-fill（2h 周期）** | write 段一站式 → 飞书回填文案字段 |
> | 4 | 全齐 | 跳过 | — |
>
> 用户填好基础信息（国家+竞品+产品+品牌）→ 启用两个定时器 → **完全甩手**，第二天回来看飞书表已全部回填。
